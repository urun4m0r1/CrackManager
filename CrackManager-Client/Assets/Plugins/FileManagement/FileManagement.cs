 ...
 * 
 * v1.8 Feature:
 * - BugFix: Deleted the "Default" option from FM_StringMode (not compatible with WSA applications).
 * - BugFix: System.Text.Encoding modes not supported on WSA 8.1 (Skipped to allow only "Fast" mode).
 * - New Feature: AddLine method allows to add text lines to an existing file.
 * - New Feature: Includes FileManagement 1.7 and OpenWavParser 1.0.
 */
...
public static class FileManagement
{
    ...
    public static FM_StringMode stringConversion = FM_StringMode.Fast;
    ...
    public static void SaveRawFile(string name, byte[] content, bool enc = false, bool fullPath = false)
    {
        if (name != "")
        {
            ...
            if (content != null)
            {
                ...
                Debug.LogError("<b>[FileManagement.SaveRawFile]</b> Exception: Trying to save null data.");
        }
        else
            Debug.LogError("<b>[FileManagement.SaveRawFile]</b> Can't save an unnamed file.");
    }
    ...
    public static byte[] ReadRawFile(string name, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        if (name != "")
        {
            ...
            if (FileExists(path, checkSA, fullPath))
            {
                ...
                Debug.LogWarning("<b>[FileManagement.ReadRawFile]</b> File not found: " + path);
            ...
            Debug.LogError("<b>[FileManagement.ReadRawFile]</b> Can't read an unnamed file.");
        return contentArray;
    }
    ...
    public static void DeleteFile(string name, bool fullPath = false)
    {
        if (FileExists(name, false, fullPath))
        {
            ...
            Debug.LogWarning("<b>[FileManagement.DeleteFile]</b> File not found: " + name);
    }
    ...
    public static void SaveRawFile(string name, byte[] content, bool enc = false, bool fullPath = false)
    {
        if (name != "")
        {
            ...
            {
                Debug.LogError("<b>[FileManagement.SaveRawFile]</b> Exception: Trying to save null data.");
                //ShowMessage("<b>[FileManagement.SaveRawFile]</b> Exception: Trying to save null data.");
            }
        }
        else
        {
            Debug.LogError("<b>[FileManagement.SaveRawFile]</b> Can't save an unnamed file.");
            //ShowMessage("<b>[FileManagement.SaveRawFile]</b> Can't save an unnamed file.");
        }
    }
    ...
    public static byte[] ReadRawFile(string name, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        if (name != "")
        {
            ...
            if (FileExists(path, checkSA, fullPath))
            {
                ...
                if (path.Contains("://") && Application.platform == RuntimePlatform.WebGLPlayer)
                {
                    ...
                    {
                        Debug.LogWarning("<b>[FileManagement.ReadRawFile]</b> File was deleted in server side: " + path);
                        //ShowMessage("<b>[FileManagement.ReadRawFile]</b> File was deleted in server side: " + path);
                    }
                }
                else
                {
            ...
            {
                Debug.LogWarning("<b>[FileManagement.ReadRawFile]</b> File not found: " + path);
                //ShowMessage("<b>[FileManagement.ReadRawFile]</b> File not found: " + path);
            }
        ...
        {
            Debug.LogError("<b>[FileManagement.ReadRawFile]</b> Can't read an unnamed file.");
            //ShowMessage("<b>[FileManagement.ReadRawFile]</b> Can't read an unnamed file.");
        }
        return contentArray;
    }
    ...
    public static void DeleteFile(string name, bool fullPath = false)
    {
        ...
        {
            Debug.LogWarning("<b>[FileManagement.DeleteFile]</b> File not found: " + name);
            //ShowMessage("<b>[FileManagement.DeleteFile]</b> File not found: " + name);
        }
    }
    ...
    private static bool CheckNameOnIndex(string name, string type)
    {
        ...
        if(blocks == null)
        {
            ...
            if (Application.platform == RuntimePlatform.WebGLPlayer)
            {
                ...
                {
                    blocks = new string[0];
                    Debug.LogWarning("<b>[FileManagement.CheckNameOnIndex]</b> Index file not found: " + indexPath);
                    //ShowMessage("<b>[FileManagement.CheckNameOnIndex]</b> Index file not found: " + indexPath);
                }
            }
            else
            {
    ...
    public static AudioClip ImportAudio(string file, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        if(fileExt == ".wav")
        {
            ...
            AudioClip clip = OpenWavParser.ByteArrayToAudioClip(wavFile, GetFileNameWithoutExtension(file));
            return clip;
        }
        else
        {
            Debug.LogError("<b>[FileManagement.ImportAudio]</b> " + fileExt + " format not supported in this platform.");
            return null;
        }
    }
    ...
    public static void SaveRawFile(string name, byte[] content, bool enc = false, bool fullPath = false)
    {
        if (name != "")
        {
            ...
            Debug.LogError("<b>[FileManagemnt.SaveRawFile]</b> Can't save an unnamed file.");
    }
            ...
            if (FileExists(path, checkSA, fullPath))
            {
                ...
                if (path.Contains("://") && Application.platform == RuntimePlatform.Android)
                {
                    ...
                    if (string.IsNullOrEmpty(www.error))
                        ...
                        Debug.LogError("<b>[FileManagement.ReadRawFile]</b> WWW error: " + www.error);
                }
                else
                {
                    // Read content normally:
    ...
    public static void SaveAudio(string name, AudioClip clip, bool enc = false, bool fullPath = false)
    {
        ...
        if (!name.EndsWith(".wav") && !name.EndsWith(".WAV"))
            name += ".wav";
        SaveRawFile(name, wavFile, enc, fullPath);
    }
    ...
    public static T ReadFile<T>(string name, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        {
            Debug.LogError("<b>[FileManagement.ReadFile]</b> Exception: FormatException - Trying to read data in the wrong format. (" + typeof(T) + "): " + name);
        }
    ...
    public static void SaveArray<T>(string name, T[] content, char separator = (char)0x09, bool enc = false, bool fullPath = false)
    {
        ...
        if (content.Length > 0)
            ...
            Debug.LogError("<b>[FileManagement.SaveArray]</b> Trying to save empty array: " + name);
    }
    /// <summary>Saves arrays or lists of one dimension</summary>
    ...
    public static void DeleteDirectory(string name, bool fullPath = false)
    {
        ...
        {
            Debug.LogWarning("<b>[FileManagement.DeleteDirectory]</b> Can't delete, directory not found: " + name);
        }
    }
    ...
    public static void EmptyDirectory(string name = "", bool filesOnly = true, bool fullPath = false)
    {
        ...
        {
            Debug.LogWarning("<b>[FileManagement.EmptyDirectory]</b> Can't delete folder content, folder not found: " + name);
        }
    }
    ...
    public static string[] ListFiles(string folder, bool checkSA = true, bool fullPath = false)
    {
        ...
        try
        {
            ...
            // Error message:
            if (result == null)
                Debug.LogWarning("<b>[FileManagement.ListFiles]</b> Can't read folder content, folder not found: " + folder);

            return result;
        }
        catch (System.Exception e)
        {
            Debug.LogError("<b>[FileManagement.ListFiles]</b> Error: " + e.Message);
            return result;
        }
    }
    ...
    public static string[] ListDirectories(string folder, bool checkSA = true, bool fullPath = false)
    {
        ...
        try
        {
            ...
            // Error message:
            if (result == null)
                Debug.LogWarning("<b>[FileManagement.ListDirectories]</b> Can't read folder content, folder not found: " + folder);
            return result;
        }
        catch (System.Exception e)
        {
            Debug.LogError("<b>[FileManagement.ListDirectories]</b> Error: " + e.Message);
            return result;
        }
    }
    ...
    public static System.Collections.Generic.List<byte[]> ReadDirectoryContent(string folder, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        {
            Debug.LogWarning("<b>[FileManagement.ReadDirectoryContent]</b> Can't read folder content, folder not found: " + folder);
        }
        return content;
    }
    ...
    public static void CopyFile(string source, string dest, bool checkSA = true, bool fullPathSource = false, bool fullPathDest = false)
    {
        if (source != "" && dest != "")
        {
            ...
            {
                Debug.LogWarning("<b>[FileManagement.CopyFile]</b> Source file not found: " + source);
            }
        }
    }
    ...
    public static void CopyDirectory(string source, string dest, bool checkSA = true, bool fullPathSource = false, bool fullPathDest = false)
    {
        if (source != "" && dest != "")
        {
            ...
            {
                Debug.LogWarning("<b>[FileManagement.CopyDirectory]</b> Source path not found: " + source);
            }
        }
    }
    ...
    public static void Move(string source, string dest, bool fullPathSource = false, bool fullPathDest = false)
    {
        if (source != "" && dest != "")
        {
            ...
            if (FileExists(source, false, fullPathSource) || DirectoryExists(source, false, fullPathSource))
            {
                ...
                Debug.LogWarning("<b>[FileManagement.Move]</b> Source file not found: " + source);
        }
    }
    ...
    public static void Rename(string source, string dest, bool fullPathSource = false, bool fullPathDest = false)
    {
        ...
        if (FileExists(source, false, fullPathSource) || DirectoryExists(source, false, fullPathSource))
            ...
            Debug.LogWarning("<b>[FileManagement.Rename]</b> Source file not found: " + source);
    }
...
#if !(UNITY_WINRT_8_1 || UNITY_WP_8_1 || UNITY_WSA_8_1)
            case FM_StringMode.ASCII:
                result = System.Text.Encoding.ASCII.GetBytes(content.ToArray());
                break;
            case FM_StringMode.UTF32:
                result = System.Text.Encoding.UTF32.GetBytes(content.ToArray());
                break;
            case FM_StringMode.UTF7:
                result = System.Text.Encoding.UTF7.GetBytes(content.ToArray());
                break;
            case FM_StringMode.BigEndianUnicode:
                result = System.Text.Encoding.BigEndianUnicode.GetBytes(content.ToArray());
                break;
            case FM_StringMode.Unicode:
                result = System.Text.Encoding.Unicode.GetBytes(content.ToArray());
                break;
            case FM_StringMode.UTF8:
                result = System.Text.Encoding.UTF8.GetBytes(content.ToArray());
                break;
#endif
            default:
    ...
    ********************************/
    ...
    {
        Debug.Log("<b>[FileManagement.Save]</b> This method has no effect, data is already saved.");
    }
    ...
    ********************************/
    ...
    {
        Debug.LogError("<b>[FileManagement.OpenFolder]</b> Not supported in this platform.");
    }
#else
    public static void OpenFolder(string path = "", bool fullPath = false)
    {
    ...
    {
        Debug.LogError("<b>[FileManagement.ObjectToByteArray]</b> Not supported in this platform.");
        return null;
    }
    /// <summary>Deseerialize object (works with default & user defined types only)</summary>
    public static object ByteArrayToObject(byte[] arrBytes)
    {
        Debug.LogError("<b>[FileManagement.ByteArrayToObject]</b> Not supported in this platform.");
        return null;
    }
#else
    /// <summary>Serialize object (works with default & user defined types only)</summary>
    ...
    {
        Debug.LogError("<b>[FileManagement.ListLogicDrives]</b> Not supported in this platform.");
        return null;
    }
#else
    public static string[] ListLogicalDrives()
    ...
    public static void InstallLocalApk(string file, bool fullPath = false)
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            {
                Debug.Log("<b>[FileManagement.InstallLocalApk]</b> The requested APK file wasn't found.");
            }
        }
        else
        {
            Debug.Log("<b>[FileManagement.InstallLocalApk]</b> Please export to some Android device.");
        }
    }
#else
    public static void InstallLocalApk(string file, bool fullPath = false)
    {
        Debug.LogError("<b>[FileManagement.InstallLocalApk]</b> Not supported in this platform.");
    }
#endif
}
...
public class FM_IniFile
{
    ...
    public void LoadNewIniFile(string name, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        {
            Debug.LogWarning("<b>[FM_IniFile.LoadNewIniFile]</b> The requested INI file wouldn't be parsed.");
        }
    }
    ...
    public void SetKey<T>(string key, T value, string section = "")
    {
        if (KeyExists(key, section))
            ...
            Debug.LogWarning("<b>[FM_IniFile.SetKey]</b> Key not found.");
    }
    ...
    public void RemoveKey(string key, string section = "")
    {
        ...
        if (_iniFile.ContainsKey(_key))
            ...
            Debug.LogWarning("<b>[FM_IniFile.RemoveKey]</b> Key not found.");
    }
